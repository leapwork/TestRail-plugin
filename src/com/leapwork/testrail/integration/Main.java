...
public class Main {
	...
	public static void main(String[] args) throws IOException, APIException {
		...
		logger.info(String.format("Passed parameters:\n%1$s\n%2$s\n%3$s\n%4$s\n%5$s\n%6$s\n%7$s\n%8$s\n",
				String.format("Run Id: %1$s", args[0]), String.format("TestRail URL: %1$s", args[1]),
				String.format("TestRail User: %1$s", args[2]), String.format("Leapwork Controller URL: %1$s", args[4]),
				String.format("Schedule Id: %1$s", args[5]), String.format("Time Delay in seconds: %1$s", args[6]),
				String.format("Done Status is interpreted as status number: %1$s", args[7]),
				String.format("api accesskey", args[8])).replace("\n", Messages.NEW_LINE));
